...
/** Hash the static L1 specialization while excluding per-call values. */
inline uint64_t hbg_argument_structure_hash(const ChipStorageTaskArgs &args) noexcept {
    if (!hbg_argument_snapshot_counts_valid(args)) return 0;
...
/** Compare the static specialization while permitting new tensor addresses and scalar values. */
inline bool hbg_argument_structures_equal(const ChipStorageTaskArgs &lhs, const ChipStorageTaskArgs &rhs) noexcept {
    if (!hbg_argument_snapshot_counts_valid(lhs) || !hbg_argument_snapshot_counts_valid(rhs) ||
        lhs.tensor_count_ != rhs.tensor_count_ || lhs.scalar_count_ != rhs.scalar_count_) {
        return false;
    }
    ...
    for (int32_t index = 0; index < lhs.tensor_count_; ++index) {
        ...
        const ChipTensor &right = rhs.tensor(index);
        if (left.ndims == 0 || left.ndims > MAX_TENSOR_DIMS || right.ndims == 0 || right.ndims > MAX_TENSOR_DIMS ||
            left.buffer.size != right.buffer.size || left.owner_task_id.raw != right.owner_task_id.raw ||
            left.start_offset != right.start_offset || left.version != right.version || left.ndims != right.ndims ||
            left.dtype != right.dtype || left.manual_dep != right.manual_dep ||
            left.is_contiguous != right.is_contiguous || left.address_space != right.address_space ||
            left.extent_elem_cache != right.extent_elem_cache) {
            return false;
        }
        for (uint32_t dim = 0; dim < left.ndims; ++dim) {
            if (left.shapes[dim] != right.shapes[dim] || left.strides[dim] != right.strides[dim]) return false;
