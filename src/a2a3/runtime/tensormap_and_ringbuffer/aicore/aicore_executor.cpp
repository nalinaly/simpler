...
// L1 borrows the physical cores from the caller's graph.  Publishing EXITED is
// only an acknowledgement that this wrapper observed STOP; it is not permission
// to return.  AICPU closes the fast-path register window and then releases this
// per-core word.  ld_dev is the A2/A3 bypass-load path used for atomic control
// observation, and the final DSB keeps wrapper retirement after that hand-off.
...
__aicore__ __attribute__((weak)) void aicore_execute(__gm__ Runtime *runtime, int block_idx, CoreType core_type) {
    ...
    __gm__ L1AicoreReport *l1_reports = reinterpret_cast<__gm__ L1AicoreReport *>(runtime->dev.l1_aicore_reports_addr_);
