...
// Private retains one region map per replay worker. The replacement shared PA
// backend owns a single direct-symbol sidecar initialized by AICPU; per-worker
// reset/retire must never touch it.
...
dist_tensor_map_lookup_for_task(__gm__ DistCore &worker, const TensorRef &tensor, int32_t consumer_task_id) {
    ...
    // CPU-sim scalar access and generic Submit have no PA task-shape proof.
    // They must not silently reinterpret the direct-symbol table as a region
    // map or fall back to a private replica.
