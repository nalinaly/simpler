...
bool shared_claim_candidate_rank(uint32_t core, SharedPaTaskKind kind, uint32_t &candidate_rank) {
...
bool shared_claim_attempted(uint32_t core, uint32_t task_id, SharedPaTaskKind kind, uint32_t participation_interval) {
    uint32_t candidate_rank = 0;
    return shared_claim_candidate_rank(core, kind, candidate_rank) &&
           fdwic_shared_claim_participates(task_id, candidate_rank, participation_interval);
}
...
bool shared_claim_tournament_group(uint32_t core, SharedPaTaskKind kind, uint32_t &group) {
    uint32_t candidate_rank = 0;
    if (!shared_claim_candidate_rank(core, kind, candidate_rank)) return false;
    group = candidate_rank % shared_claim_tournament_groups(kind);
    return true;
}

bool shared_claim_tournament_group_has_participant(
    SharedPaTaskKind kind, uint32_t task_id, uint32_t group, uint32_t participation_interval
) {
    const uint32_t workers = kind == SharedPaTaskKind::Alloc                              ? kFdwicSharedWorkers :
                             kind == SharedPaTaskKind::Qk || kind == SharedPaTaskKind::Pv ? kFdwicSharedAicWorkers :
                                                                                            kFdwicSharedAivWorkers;
    const uint32_t groups = shared_claim_tournament_groups(kind);
    for (uint32_t candidate_rank = 0; candidate_rank < workers; ++candidate_rank) {
        if (candidate_rank % groups == group &&
            fdwic_shared_claim_participates(task_id, candidate_rank, participation_interval)) {
            return true;
        }
    }
...
bool expand_shared_records(
    uint32_t core, uint32_t participation_interval, const FdwicSwimlaneRecord *generic_records, uint32_t generic_count,
    ...
    for (uint32_t task_id = 0; task_id < kFdwicSharedTracePhase1TaskCount; ++task_id) {
        ...
        const bool attempted = (endpoints.claim_begin & kFdwicSharedClaimAttemptedBit) != 0;
        const uint32_t encoded_interval =
            fdwic_shared_claim_participation_interval_from_bits(endpoints.submit_begin, endpoints.submit_end);
        const bool expected_attempted = shared_claim_attempted(core, task_id, kind, participation_interval);
        const uint64_t claim_begin = endpoints.claim_begin & ~kFdwicSharedEndpointMetadataBit;
        const uint64_t claim_end = endpoints.claim_end_and_winner & ~kFdwicSharedEndpointMetadataBit;
        const uint64_t submit_begin = endpoints.submit_begin & ~kFdwicSharedEndpointMetadataBit;
        const uint64_t submit_end = endpoints.submit_end & ~kFdwicSharedEndpointMetadataBit;
        if (encoded_interval != participation_interval || attempted != expected_attempted || claim_begin == 0 ||
            submit_begin == 0 || claim_end < claim_begin || submit_end < submit_begin || submit_begin > claim_begin ||
            claim_end > submit_end || (winner && !attempted)) {
            return false;
        }
...
bool validate_and_write_shared_core(
    ...
    int32_t expected_block, int32_t expected_lane, uint32_t level, uint32_t participation_interval, std::ofstream &out,
    bool &first, TraceSummary &observed, std::vector<uint32_t> &winner_counts,
        ...
        if (phase == FdwicSwimlanePhase::Atomic) {
            ...
            if (valid) {
                ...
                if (phase == FdwicSwimlanePhase::Claim) {
                    ...
                    const bool attempted = shared_claim_attempted(core, task_id, kind, participation_interval);
                    valid =
...
extern "C" int fdwic_swimlane_host_export(Runtime *runtime) {
    ...
    FdwicSharedSubmitClaimRecord participation_probe{};
    if (runtime->host_api.copy_from_device(
            &participation_probe, reinterpret_cast<const void *>(records_base), sizeof(participation_probe)
        ) != 0) {
        LOG_ERROR("fdwic shared swimlane participation identity D2H copy failed");
        std::remove(temporary_path.c_str());
        return -1;
    }
    const uint32_t participation_interval = fdwic_shared_claim_participation_interval_from_bits(
        participation_probe.submit_begin, participation_probe.submit_end
    );
    if (!fdwic_shared_claim_participation_interval_supported(participation_interval) ||
        (participation_probe.claim_begin & ~kFdwicSharedEndpointMetadataBit) == 0 ||
        (participation_probe.claim_begin & kFdwicSharedClaimAttemptedBit) == 0) {
        LOG_ERROR(
            "fdwic shared swimlane has invalid participation identity: interval=%u claim_begin=%llu encoded=0x%llx",
            participation_interval, static_cast<unsigned long long>(participation_probe.claim_begin),
            static_cast<unsigned long long>(participation_probe.claim_end_and_winner)
        );
        std::remove(temporary_path.c_str());
        return -1;
    }
    ...
    out << "    \"claim_participation_interval\": " << participation_interval << ",\n";
#endif
    ...
    for (uint32_t c = 0; c < header->num_cores; c++) {
        ...
        if (!expand_shared_records(
                c, participation_interval, decoded_scratch.data(), count, submit_claim_scratch.data(), logical_scratch
            )) {
        ...
        if (!validate_and_write_shared_core(
                ...
                expected_lanes[c], level, participation_interval, out, first, observed, winner_counts, root_group_counts
            )) {
            LOG_ERROR(
    ...
    for (uint32_t task_id = 0; task_id < kFdwicSharedTracePhase1TaskCount; ++task_id) {
        ...
        if (level >= kFdwicAtomicSwimlaneLevel) {
            ...
            for (uint32_t group = 0; group < groups; ++group) {
                const bool active = shared_claim_tournament_group_has_participant(
                    shared_pa_task_kind(task_id), task_id, group, participation_interval
                );
                const uint32_t expected = active ? 1U : 0U;
                if (root_group_counts[task_id][group] != expected) {
                    LOG_ERROR(
                        "fdwic shared Claim tournament task %u group %u expects %u root contenders; got %u", task_id,
                        group, expected, root_group_counts[task_id][group]
                    );
                    return fail_export();
                }
            }
