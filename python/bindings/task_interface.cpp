...
NB_MODULE(_task_interface, m) {
    ...
    nb::class_<Tensor>(m, "Tensor")
        ...
        .def_static(
            "make_strided",
            [](uint64_t data, nb::tuple shapes, nb::tuple strides, DataType dtype, bool child_memory) -> Tensor {
                const size_t rank = nb::len(shapes);
                if (rank == 0 || rank > MAX_TENSOR_DIMS || nb::len(strides) != rank) {
                    throw std::invalid_argument(
                        "Tensor.make_strided: shapes/strides must have the same rank in [1, MAX_TENSOR_DIMS]"
                    );
                }
                ...
                for (size_t i = 0; i < rank; ++i) {
                    ...
                    if (stride_values[i] == 0) {
                        throw std::invalid_argument("Tensor.make_strided: every stride must be positive");
                    }
                ...
                Tensor tensor = make_tensor_external(
                    ...
                    dtype, /*manual_dep=*/false, /*version=*/0, child_memory ? 1 : 0
                );
                ...
                for (size_t reverse = rank; reverse > 0; --reverse) {
                    ...
                    if (!empty) {
                        ...
                        if (span > std::numeric_limits<uint64_t>::max() - extent) {
                            throw std::overflow_error("Tensor.make_strided: tensor extent overflows uint64_t");
                        }
                        extent += span;
                    }
                    if (shape_values[i] != 0 &&
                ...
                if (extent > std::numeric_limits<uint64_t>::max() / element_size) {
                    throw std::overflow_error("Tensor.make_strided: tensor byte extent overflows uint64_t");
                }
            ...
            "Create a positive-stride Tensor whose buffer base is data. The data pointer is treated as the view "
