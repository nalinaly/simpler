...
# -----------------------------------------------------------------------------------------------------------
"""MB-6 deterministic GM output heap — atomic-free allocation + frontier reclaim backpressure.

Reuses the benchmark_bgemm workload (cross-task dependencies) with a small heap
to trigger reclaim backpressure. Verifies no premature recycling (golden passes)
and deterministic FATAL on too-small heap (no silent overwrite).

Criteria (docs MB-6):
  - vend[N] identical across cores (deterministic allocation)
  - Golden passes (no premature recycle → no data corruption)
  - Heap too small / H too small → deterministic FATAL, never silent overwrite

Env knobs:
  PTO_DIST_HEAP_MB=N         — GM output heap ring size (small → triggers backpressure)
  PTO_DIST_H=N               — dependency span bound (large → wider live window)
...
class TestMB6HeapReclaim(SceneTestCase):
    ...
    CASES = [
        {
            # Normal: heap is large enough, golden must pass.
            "name": "Normal",
            "platforms": ["a5sim", "a5"],
        ...
        {
            # Many tasks with dependencies: stresses heap reclaim window.
            "name": "Heavy",
            "manual": True,
            "platforms": ["a5sim"],
