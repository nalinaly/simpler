...
#if PTO_FDWIC_SHARED_MAP
PA_DEVICE FdwicOutputRef MakePlainSharedOutputRef(
    uint32_t producer_task_id, uint32_t output_slot
) {
    if (producer_task_id >= kMaxTasks ||
        output_slot >= kSharedOutputMaxPerTask) {
        return InvalidSharedOutputRef();
    }
    return FdwicOutputRef{
        static_cast<int32_t>(producer_task_id),
        static_cast<int16_t>(output_slot), 0, 0, 0, 0,
    };
}

PA_DEVICE bool BindSharedPaTaskForRandomAccess(
    PaOrchestrationState &orch, uint32_t batch,
    uint32_t batch_start, uint32_t task_id,
    TaskKind kind, uint32_t group_index
) {
    // shared output handle 是确定性的 (producer_task_id, output_slot)，
    // 不依赖本 worker 是否顺序回放过 producer。这里从不可变 PA task
    // 身份重建当前 callback 真正会读取的最小 orchestration 状态，供后续
    // 任意 Scalar 取得 task 后构参；不触碰 TensorMap，也不发布 task。
    if (batch >= orch.context_lens.shapes[0] ||
        task_id < batch_start || kind >= TaskKind::Count) {
        return false;
    }

    BeginPaBatchForCallback(orch, batch);
    SharedPaBatchPlan plan{};
    if (!BuildSharedPaBatchPlan(
            orch.current_sequence, batch_start, plan
        )) {
        return false;
    }
    const uint32_t task_offset = task_id - batch_start;
    SharedPaPlannedTask planned{};
    if (!SharedPaPlannedTaskAt(plan, task_offset, planned) ||
        planned.kind != kind ||
        planned.group_index != group_index) {
        return false;
    }
    if (kind == TaskKind::Alloc) {
        return task_id == batch_start && group_index == 0;
    }

    PreparePaBlockGroup(
        orch, static_cast<uint64_t>(group_index) *
                  kPaBlocksPerRequest
    );
    if (kind == TaskKind::Sf) {
        orch.qk_scores = MakePlainSharedOutputRef(
            batch_start + SharedPaTaskOffset(
                TaskKind::Qk, group_index
            ),
            0
        );
    } else if (kind == TaskKind::Pv) {
        orch.sf_probs = MakePlainSharedOutputRef(
            batch_start + SharedPaTaskOffset(
                TaskKind::Sf, group_index
            ),
            0
        );
    } else if (kind == TaskKind::Up) {
        const uint32_t sf_task =
            batch_start + SharedPaTaskOffset(
                TaskKind::Sf, group_index
            );
        const uint32_t pv_task =
            batch_start + SharedPaTaskOffset(
                TaskKind::Pv, group_index
            );
        orch.sf_max = MakePlainSharedOutputRef(sf_task, 1);
        orch.sf_sum = MakePlainSharedOutputRef(sf_task, 2);
        orch.pv_output = MakePlainSharedOutputRef(pv_task, 0);
        orch.accumulated_output =
            MakePlainSharedOutputRef(batch_start, 0);
        orch.accumulated_sum =
            MakePlainSharedOutputRef(batch_start, 1);
        orch.accumulated_max =
            MakePlainSharedOutputRef(batch_start, 2);
    }
    return true;
}
#endif
