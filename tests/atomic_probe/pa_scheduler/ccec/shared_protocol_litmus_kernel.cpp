...
__aicore__ inline bool PublishLitmusGate(
    __gm__ pa_scheduler::SchedulerState *state, int32_t task_id
) {
    if (state == nullptr || task_id < 0 ||
        task_id >= static_cast<int32_t>(
            pa_scheduler::kMaxTasks
        )) {
        return false;
    }
    CcecOps::StoreBarrier();
    return CcecOps::CompareExchange(
               &state->tasks[
...
__aicore__ inline bool WaitForLitmusGate(
    __gm__ pa_scheduler::SchedulerState *state,
    int32_t task_id, pa_scheduler::LocalStats &stats
) {
    if (state == nullptr || task_id < 0 ||
        task_id >= static_cast<int32_t>(
            pa_scheduler::kMaxTasks
    ...
    __gm__ volatile int64_t *gate =
        &state->tasks[
             static_cast<uint32_t>(task_id)
         ].deps_prepared;
    int64_t observed = CcecOps::Load(gate);
    if (observed == task_id) {
        return true;
    }
    if (observed != -1) {
...
__aicore__ inline void RunSharedProtocolParticipant(
    ...
    RunReaderReclaimParticipant(state, control, worker);
}
