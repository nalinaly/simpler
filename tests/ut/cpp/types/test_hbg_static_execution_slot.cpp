...
FakeStaticArena *g_arena = nullptr;

int fake_setup_static_arena(size_t gm_heap, size_t shared_memory, size_t runtime_arena) {
    ++g_arena->setup_calls;
    g_arena->gm_heap_capacity = gm_heap;
    g_arena->shared_memory_capacity = shared_memory;
    g_arena->runtime_arena_capacity = runtime_arena;
    return g_arena->setup_rc;
}

void *fake_acquire_gm_heap() {
    ++g_arena->acquire_calls;
    return g_arena->gm_heap;
}

void *fake_acquire_shared_memory() {
    ++g_arena->acquire_calls;
    return g_arena->shared_memory;
}

void *fake_acquire_runtime_arena() {
    ++g_arena->acquire_calls;
    return g_arena->runtime_arena;
}

int fake_freeze_static_arena(
    const void *gm_heap, size_t gm_heap_capacity, const void *shared_memory, size_t shared_memory_capacity,
    const void *runtime_arena, size_t runtime_arena_capacity
) {
    ++g_arena->freeze_calls;
    EXPECT_EQ(gm_heap, g_arena->gm_heap);
    EXPECT_EQ(gm_heap_capacity, g_arena->gm_heap_capacity);
    EXPECT_EQ(shared_memory, g_arena->shared_memory);
    EXPECT_EQ(shared_memory_capacity, g_arena->shared_memory_capacity);
    EXPECT_EQ(runtime_arena, g_arena->runtime_arena);
    EXPECT_EQ(runtime_arena_capacity, g_arena->runtime_arena_capacity);
    return g_arena->freeze_rc;
}

HostApi make_host_api() {
    HostApi api{};
    api.setup_static_arena = fake_setup_static_arena;
    api.acquire_pooled_gm_heap = fake_acquire_gm_heap;
    api.acquire_pooled_gm_sm = fake_acquire_shared_memory;
    api.acquire_pooled_runtime_arena = fake_acquire_runtime_arena;
    api.freeze_static_arena = fake_freeze_static_arena;
    return api;
}
...
protected:
    void SetUp() override { g_arena = &arena_; }
    void TearDown() override { g_arena = nullptr; }
...
TEST_F(HbgStaticExecutionSlotTest, MissingFreezeCapabilityHasNoPlatformSideEffects) {
    api_.freeze_static_arena = nullptr;
    HbgPreparedStaticExecutionSlot slot{};
